...
grid cell_grid width: 100 height: 100 neighbors: 8 {
    ...
    // Aspect for display without grid lines (no border)
    aspect no_grid {
        draw shape color: is_flooded ? rgb(0, 0, 255, flood_intensity) : color border: #transparent;
    }
}
...
global {
    ...
    // Speed parameters (in m/s) - Matching NetLogo values
    float human_speed_std <- 0.3;
    float human_speed_min <- 1.4;  // m/s - Matching NetLogo
    float human_speed_max <- 7.0;  // m/s - Matching NetLogo
    float human_speed_avg <- (human_speed_min + human_speed_max) / 2.0;  // Average: 4.2 m/s
    ...
    // Car parameters - Matching NetLogo values
    float car_speed_std <- 0.5;
    float car_speed_min <- 1.4;   // m/s - Matching NetLogo
    float car_speed_max <- 36.1;  // m/s - Matching NetLogo
    float car_speed_avg <- (car_speed_min + car_speed_max) / 2.0;  // Average: 18.75 m/s
    ...
    init {
        ...
        // ROAD NETWORK CREATION FLOW:
        // 1. cleaned_roads: list<geometry> - cleaned road segments (edges, NOT vertices)
        // 2. road: list<agent> - road agents created from cleaned_roads geometries
        // 3. graph: created from road agents using as_edge_graph()
        // 4. vertices: extracted from graph (intersection points, NOT stored in cleaned_roads)
        ...
        // cleaned_roads = list of geometry segments (edges), NOT vertices
        // VÍ DỤ: cleaned_roads = [
        //   line([point(100,200), point(300,200)]),  // Edge 1: đoạn đường A→B
        //   line([point(300,200), point(300,100)]),  // Edge 2: đoạn đường B→C
        //   line([point(300,100), point(100,100)])   // Edge 3: đoạn đường C→D
        // ]
        // Mỗi phần tử là một geometry (line), không phải vertices (points)
        ...
        if (empty(cleaned_roads)) {
            ...
            // Create graph from road agents - vertices are automatically extracted from intersections
            ...
            // vertices = intersection points extracted from graph, NOT from cleaned_roads
        ...
        loop i from: 0 to: data.rows - 1 {
            create shelter {
                ...
                name <- "";  // Empty name to prevent default "shelter 1, 2, 3..." display
                current_occupants <- 0;
            }
        }
        ...
        loop i from: 0 to: locals_number - 1 {
            create people {
                ...
                // Initialize speed using normal distribution like NetLogo
                speed <- gauss(human_speed_avg, human_speed_std);
        ...
        simulation_start_time <- machine_time;
        write "";
        write "=== SIMULATION STARTED ===";
        write "Start time: " + string(simulation_start_time) + " ms";
        write "Initialization completed at cycle " + cycle;
    }
    ...
    reflex check_tsunami_end when: cycle >= tsunami_approach_time and not simulation_complete {
        ...
        if (tsunami_passed_through or all_segments_stopped or map_coverage >= 100.0) {
            ...
            // Calculate and log simulation timing
            simulation_end_time <- machine_time;
            simulation_duration <- simulation_end_time - simulation_start_time;
            write "";
            write "=== SIMULATION TIMING ===";
            write "Start time: " + string(simulation_start_time) + " ms";
            write "End time: " + string(simulation_end_time) + " ms";
            write "Total duration: " + string(simulation_duration) + " ms (" + 
                  string(simulation_duration / 1000.0) + " seconds)";
            write "Total cycles: " + cycle;
            write "Average time per cycle: " + string(simulation_duration / cycle) + " ms";
...
species shelter {
    ...
    aspect default {
        ...
        // Name display removed - set name to empty string when creating shelters
...
species people skills: [moving] {
    ...
    reflex move when: !is_dead and !is_safe and (cycle mod update_frequency = 0) {
        switch type {
            match "local" {
                // Randomize speed each step like NetLogo (using normal distribution with avg and std)
                speed <- gauss(human_speed_avg, human_speed_std);
                if (speed < human_speed_min) { speed <- human_speed_min; }
                if (speed > human_speed_max) { speed <- human_speed_max; }
                ...
                // GOTO ON GRAPH: 
                // - goto on: nhận GRAPH (không phải edge hay vertices trực tiếp)
                // - GAMA tự động snap location và target đến vertices gần nhất
                // - Tìm đường đi ngắn nhất qua các vertices
                // - Agent di chuyển dọc theo edges nối giữa vertices
...
species car skills: [moving] {
    ...
    bool is_safe <- false;
    rgb color <- #brown;
    // Initialize speed using normal distribution like NetLogo (clamped to min/max)
    float speed <- min([max([gauss(car_speed_avg, car_speed_std), car_speed_min]), car_speed_max]);
    ...
    // CRITICAL FIX: Track previous location to detect stuck/spinning cars
    point previous_location <- location;
    int stuck_counter <- 0;  // Count consecutive cycles without movement
    ...
    // NEW: Check for agents (cars and people) ahead in forward direction
    // Returns true if there are blocking agents ahead, false otherwise
    bool check_agents_ahead(point target, float check_distance) {
        // Calculate forward direction vector
        point direction_vector <- target - location;
        float direction_length <- sqrt(direction_vector.x * direction_vector.x + direction_vector.y * direction_vector.y);
        
        // If direction is too short, no need to check
        if (direction_length < 0.1) {
            return false;
        }
        
        // Normalize direction vector
        point forward_direction <- {direction_vector.x / direction_length, direction_vector.y / direction_length};
        
        // Get all nearby agents
        list<people> nearby_people <- get_nearby_people(check_distance * 2.0);  // Check wider area
        list<car> nearby_cars <- get_nearby_cars(check_distance * 2.0);
        
        // Use dot product to check if agents are in forward direction (within 90 degrees cone)
        // Dot product > 0 means forward direction, > 0.707 means within 45 degrees (cos(45°))
        // We use > 0 to allow 90 degrees cone (cos(90°) = 0)
        float min_dot_product <- 0.0;  // 90 degrees cone (cos(90°) = 0)
        
        // Check for cars ahead
        loop car_ahead over: nearby_cars {
            point car_direction <- car_ahead.location - location;
            float car_distance <- sqrt(car_direction.x * car_direction.x + car_direction.y * car_direction.y);
            if (car_distance > 0.1 and car_distance <= check_distance) {
                // Normalize car direction
                point car_direction_norm <- {car_direction.x / car_distance, car_direction.y / car_distance};
                
                // Calculate dot product (forward_direction · car_direction_norm)
                float dot_product <- forward_direction.x * car_direction_norm.x + forward_direction.y * car_direction_norm.y;
                
                // If dot product > 0, car is in forward direction (within 90 degrees)
                if (dot_product > min_dot_product) {
                    return true;  // Found blocking car ahead
                }
            }
        }
        
        // Check for people ahead
        loop person_ahead over: nearby_people {
            point person_direction <- person_ahead.location - location;
            float person_distance <- sqrt(person_direction.x * person_direction.x + person_direction.y * person_direction.y);
            if (person_distance > 0.1 and person_distance <= check_distance) {
                // Normalize person direction
                point person_direction_norm <- {person_direction.x / person_distance, person_direction.y / person_distance};
                
                // Calculate dot product (forward_direction · person_direction_norm)
                float dot_product <- forward_direction.x * person_direction_norm.x + forward_direction.y * person_direction_norm.y;
                
                // If dot product > 0, person is in forward direction (within 90 degrees)
                if (dot_product > min_dot_product) {
                    return true;  // Found blocking person ahead
                }
            }
        }
        
        return false;  // No blocking agents ahead
    }
    ...
    reflex enforce_constraints when: !is_dead and !is_safe {
        ...
        // CONSTRAINT 2: Ensure car stays on road/land (same logic as people)
        // CRITICAL FIX: Only enforce when car is actually off-road/land (like people do)
        // This prevents conflict with goto's vertex snapping
        if (!is_dead and (current_cell = nil or (!current_cell.is_land and !current_cell.is_road))) {
            road r <- road closest_to self;
            if (r != nil) {
                location <- r.shape.points closest_to self;
    ...
    reflex move when: !is_dead and !is_safe and (cycle mod update_frequency = 0) {
        ...
        // Randomize speed each step like NetLogo (using normal distribution with avg and std)
        speed <- gauss(car_speed_avg, car_speed_std);
        if (speed < car_speed_min) { speed <- car_speed_min; }
        if (speed > car_speed_max) { speed <- car_speed_max; }
        ...
        if (car_strategy = "always go ahead") {
            ...
            // CRITICAL FIX: Detect if car is stuck (spinning in place)
            float distance_moved <- previous_location distance_to location;
            if (distance_moved < 0.5) {  // Car moved less than 0.5m
                stuck_counter <- stuck_counter + 1;
                if (stuck_counter >= 3) {  // Stuck for 3+ cycles
                    // Force path invalidation and try alternative approach
                    path_invalidated <- true;
                    
                    // Check if current vertex is isolated (no outgoing edges)
                    point current_vertex <- simplified_road_network.vertices with_min_of (each distance_to self);
                    if (current_vertex != nil) {
                        list<point> neighbors <- neighbors_of(simplified_road_network, current_vertex);
                        if (empty(neighbors)) {
                            // Current vertex is isolated - try to find any reachable vertex
                            list<point> all_reachable <- simplified_road_network.vertices where (each distance_to self < 100.0);
                            if (!empty(all_reachable)) {
                                // Try moving to any nearby vertex to escape isolation
                                point escape_target <- all_reachable with_min_of (each distance_to target);
                                do goto target: escape_target on: simplified_road_network speed: speed;
                            } else {
                                // No reachable vertices - try full road_network as fallback
                                list<point> fallback_vertices <- road_network.vertices where (each distance_to self < 100.0);
                                if (!empty(fallback_vertices)) {
                                    point fallback_target <- fallback_vertices with_min_of (each distance_to target);
                                    do goto target: fallback_target on: road_network speed: speed;
                                }
                            }
                        } else {
                            // Vertex has neighbors but car is stuck - try moving to nearest neighbor
                            point nearest_neighbor <- neighbors with_min_of (each distance_to target);
                            do goto target: nearest_neighbor on: simplified_road_network speed: speed;
                        }
                    } else {
                        // Can't find current vertex - try nearby vertices
                        list<point> nearby_vertices <- simplified_road_network.vertices where (each distance_to self < 30.0);
                        if (!empty(nearby_vertices)) {
                            point alternative_target <- nearby_vertices with_min_of (each distance_to target);
                            do goto target: alternative_target on: simplified_road_network speed: speed;
                        }
                    }
                }
            } else {
                // Car is moving, reset stuck counter
                stuck_counter <- 0;
            }
            
            // NEW: Check for agents (cars and people) ahead in forward direction
            // This matches NetLogo's "patch-ahead 1" check
            float check_distance <- 5.0;  // Check 5m ahead (similar to NetLogo's patch-ahead)
            bool has_agents_ahead <- check_agents_ahead(target, check_distance);
            
            if (has_agents_ahead) {
                // Agents ahead - car is stuck, don't move (or move very slowly)
                speed <- car_speed_min * 0.1;  // Almost stopped (10% of min speed)
                // Don't call goto - car is stuck
            } else {
            ...
            previous_location <- copy(location);
        }
        // Strategy: Go out when congestion
            ...
            if (current_road != nil and current_road.is_flooded) {
                ...
                // NEW: Check for agents (cars and people) ahead in forward direction
                float check_distance <- 5.0;  // Check 5m ahead
                bool has_agents_ahead <- check_agents_ahead(target, check_distance);
                
                if (has_agents_ahead) {
                    // Agents ahead - car is stuck
                    cars_time_wait <- cars_time_wait + 1;
                    speed <- car_speed_min * 0.1;  // Almost stopped
                    path_invalidated <- true;
...
experiment tsunami_simulation type: gui {
    ...
    output {
        display main_display type: opengl axes: false {
            ...
            // Display grid without borders to show land/water but hide grid lines
            species cell_grid aspect: no_grid transparency: 0.3;
...
//                draw "Boats: " + length(boat) at: {x, y - 110} color: #black;
            ...
            // NETWORK_VERTICES PURPOSE:
            // - Chủ yếu để VISUALIZATION/DEBUGGING (hiển thị các điểm giao lộ)
            // - KHÔNG dùng trực tiếp cho pathfinding (dùng graph thay vì)
            // - Giúp kiểm tra topology của road network sau khi cleaned_roads
